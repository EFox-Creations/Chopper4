...
public class DefaultEventHandler {
		...
		if (stickyId != null) {
			channel.retrieveMessageById(stickyId).queueAfter(500L, TimeUnit.MILLISECONDS, message -> {
				...
				message.delete().queue(unused -> {
					channel.sendMessageEmbeds(messageEmbed).queue(newMsg -> {
	...
	public static void handleGMemJoin(GuildMemberJoinEvent event) {
		event.getGuild().getSystemChannel().sendMessageEmbeds(Embeds.getWelcomeEmbed(event.getUser())).queue();
	}

	public static void handleGMemRemove(GuildMemberRemoveEvent event) {
		event.getGuild().getSystemChannel().sendMessageEmbeds(Embeds.getLeaveEmbed(event.getUser())).queue();
	}
	...
	public static void nightlyReset(Guild g) {
		new Database().resetDailyCounts(g, new ArrayList<>(), new ArrayList<>(), new ArrayList<>(), new ArrayList<>());
	}
